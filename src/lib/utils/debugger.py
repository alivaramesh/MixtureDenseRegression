...
class Debugger(object):
  ...
  def add_blend_img(self, back, fore, img_id='blend', trans=0.7):
    # back = back[:,:,::-1]
    print(fore.max(), fore.min(), fore.mean(), fore.std(), fore.dtype)
    fore[fore<20] = 0
    fore = fore.astype(np.float64)*1.2
    fore[fore>255] = 255
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True,pi_max_idx=None,pi_max=None, img_id='default'): 
    ...
    if pi_max_idx is not None:
      pi_max_idx = list(map(int,pi_max_idx))
      pi_max = list(map(lambda x: int(100*round(x,2)),pi_max))
      if len(pi_max_idx) == 2:
        txt += '_{},{}_{}.{}'.format(pi_max_idx[0],pi_max[0],pi_max_idx[1],pi_max[1])
      else:
        txt += '_{},{}'.format(pi_max_idx[0],pi_max[0])
    font = cv2.FONT_HERSHEY_SIMPLEX
    cat_size = cv2.getTextSize(txt, font, 0.4, 2)[0]
    cv2.rectangle(
  ...
  def save_img(self, imgId='default', path='./cache/debug/'):
    #img = self.imgs[imgId][:,:,::-1]
    cv2.imwrite(path + '/{}.png'.format(imgId), img)
  ...
  def save_all_imgs(self, path='./cache/debug/', prefix='', genID=False):
    ...
    for i, v in self.imgs.items():
      save_path = path + '/{}{}.jpg'.format(prefix, i)
      #cv2.imwrite(path + '/{}{}.png'.format(prefix, i), v)
      cv2.imwrite(save_path, v[:,:,::1])
      # h,w,_ = v.shape
      # print(h,w)
      # fig,ax = plt.subplots()
      ...
      # ax.set_axis_off()
      # if i != 'multi_pose':
      #   self.add_cbar(fig,ax,_im)
      # fig.tight_layout()
      # fig.savefig(save_path)
  
  def add_cbar(self,fig,ax,_im):
