...
def ctdet_decode(heat, wh, reg=None, cat_spec_wh=False, K=100, **kwargs):
    batch, cat, height, width = heat.size()
    ...
    mdn_max_idx = kwargs.get('mdn_max_idx')
    mdn_max_pi=kwargs.get('mdn_max_pi')
    mdn_sigmas = kwargs.get('mdn_sigmas')
    
    if mdn_max_idx is not None:
      tmp = [_tranpose_and_gather_feat(mdn_max_idx[i:i+1],inds) for i in range(mdn_max_idx.shape[0])]
      mdn_max_idx = torch.cat( tmp,dim=2)
      
      tmp = [_tranpose_and_gather_feat(mdn_max_pi[i:i+1],inds) for i in range(mdn_max_pi.shape[0])]
      mdn_max_pi = torch.cat( tmp,dim=2)
      
      mdn_sigmas = _tranpose_and_gather_feat(mdn_sigmas, inds)
      topk_xs = copy.deepcopy(xs)
      topk_ys = copy.deepcopy(ys)
    ...
    #   print("mdn_sigmas.shape:",mdn_sigmas.shape)
    #   print("mdn_mu.shape:",mdn_mu.shape)
    #   print("mdn_max_pi.shape:",mdn_max_pi.shape)
      detections = torch.cat([bboxes, scores, clses,\
                            mdn_max_idx.float(),mdn_sigmas.float(),mdn_mu.float(),\
                            mdn_max_pi.float(),topk_xs.unsqueeze(2).float(),topk_ys.unsqueeze(2).float()], dim=2)
    else:
...
def multi_pose_decode(
    heat, wh, kps, reg=None, hm_hp=None, hp_offset=None, K=100, **kwargs):
  batch, cat, height, width = heat.size()
  ...
  mdn_sigmas = kwargs.get('mdn_sigmas')
  if mdn_max_idx is not None:
      #mdn_max_idx = _tranpose_and_gather_feat(mdn_max_idx,inds.repeat(2,1)).permute(2,1,0)
      tmp = [_tranpose_and_gather_feat(mdn_max_idx[i:i+1],inds) for i in range(mdn_max_idx.shape[0])]
      mdn_max_idx = torch.cat( tmp,dim=2)
  ...
  if hm_hp is not None:
      ...
      wh_factor = kwargs.get('wh_factor',.3)
      mask = (hm_kps[..., 0:1] < l) + (hm_kps[..., 0:1] > r) + \
             (hm_kps[..., 1:2] < t) + (hm_kps[..., 1:2] > b) + \
             (hm_score < thresh) + (min_dist > (torch.max(b - t, r - l) * wh_factor))
            #   + \
            #  (scores.unsqueeze(1)<.2)
  ...
  if mdn_max_idx is not None:
    detections = torch.cat([bboxes, scores, kps, clses,\
                            mdn_max_idx.float(),mdn_sigmas.float(),mdn_mu.float(),\
                            mdn_max_pi.float(),topk_xs.unsqueeze(2).float(),topk_ys.unsqueeze(2).float()], dim=2)
  else:
